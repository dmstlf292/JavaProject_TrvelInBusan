...
public class ChatClientTib extends MFrame 
implements ActionListener, Runnable {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj==bt1/*connect*/) {
			...
			try {
				FileWriter fw=new FileWriter("tib/"+System.currentTimeMillis()+".txt");
				fw.write(str);
				fw.flush();
				fw.close();
				area.setText("");
			...
			if(!flag)/*아이디 입력일때 */ {
				sendMessage(ChatProtocolTib.ID+":"+str);
				setTitle(getTitle() + "-"+str+"님 반값습니다.");
				area.setText("");
				tf3.setText("");
				tf3.requestFocus();
				flag=true;
				...
				if(i==-1 || i==0)/*전체채팅*/  {
					sendMessage(ChatProtocolTib.CHATALL+":"+str);
				}else/*귓속말*/ {
					String id=list.getSelectedItem();
					sendMessage(ChatProtocolTib.CHAT+":"+id+";"+str); // 콜론이랑 세미콜론 둘 다 사용함!! ":" & ";" 주의!!!!!!!!!!!!
				}
				tf3.setText("");
				tf3.requestFocus();
			}
	...
	public void routine(String line) {
		...
		if(cmd.equals(ChatProtocolTib.CHATLIST)) {
			//data : aaa;bbb;ccc;
			...
			while(st.hasMoreTokens()) {
				...
				cmd.equals(ChatProtocolTib.CHATALL)) {
			area.append(data+"\n");
	...
	class Message extends Frame implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==send){
				sendMessage(ChatProtocolTib.MESSAGE+
						":"+id+";"+ ta.getText());
			}
			setVisible(false);
			dispose();
	...
	class MDialog extends Dialog implements ActionListener{
		...
		public MDialog(ChatClientTib ct2,String title, String msg) {
